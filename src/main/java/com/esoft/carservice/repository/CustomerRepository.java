...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    @Query(value = "SELECT * FROM customer c WHERE (?1 IS NULL OR c.mobile_number =?1) AND\n" +
            ...
            "            (?3 IS NULL OR (SELECT u.user_id FROM user u WHERE u.email LIKE %?3% AND c.user_user_id=u.user_id )) AND\n" +
            "            (?4 = 0 OR (SELECT u.user_id FROM user u where u.user_id=?4 AND c.user_user_id=u.user_id )) AND\n" +
