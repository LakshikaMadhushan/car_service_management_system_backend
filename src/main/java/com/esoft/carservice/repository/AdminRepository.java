...
public interface AdminRepository extends JpaRepository<Admin, Long> {
    @Query(value = "SELECT * FROM admin a WHERE (?1 IS NULL OR a.mobile_number =?1) AND\n" +
            ...
            "(?3 IS NULL OR (SELECT u.user_id FROM user u WHERE u.email LIKE %?3% AND u.admin_admin_id=a.admin_id )) AND\n" +
            "(?4 = 0 OR (SELECT u.user_id FROM user u where u.user_id=?4 AND u.admin_admin_id=a.admin_id )) AND\n" +
